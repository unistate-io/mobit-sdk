...
import { bytes, createFixedBytesCodec, molecule, number } from '@ckb-lumos/codec';
...
export async function convertToTxSkeleton(
    ...
    const fetcher = async (outPoint: BaseComponents.OutPoint) => {
        console.debug(`Fetching live cell for outPoint: ${JSON.stringify(outPoint)}`);
        const cell = await collector.getLiveCell(outPoint);
        console.debug(`Fetched live cell: ${JSON.stringify(cell)}`);
        return convertLiveCell(cell, outPoint);
    };
    ...
    const transaction: import("@ckb-lumos/base").Transaction = {
        ...
        witnesses: rawTransaction.witnesses.map((witness) => {
            console.debug(`Processing witness: ${typeof witness === 'string' ? witness : 'non-string witness'}`);
    ...
    const inputCells = await Promise.all(transaction.inputs.map(input => {
        console.debug(`Fetching cell for input: ${JSON.stringify(input)}`);
        return fetcher(input.previousOutput);
    }));
    ...
    const outputCells: Array<BaseComponents.Cell> = transaction.outputs.map((output, index) => {
