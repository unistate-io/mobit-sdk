...
import { append0x, appendCkbTxWitnesses, BTCTestnetType, buildRgbppLockArgs, calculateRgbppClusterCellCapacity, calculateTransactionFee, Collector, genCreateClusterCkbVirtualTx, generateClusterCreateCoBuild, genRgbppLockScript, getSecp256k1CellDep, MAX_FEE, NoLiveCellError, RawClusterData, SECP256K1_WITNESS_LOCK_SIZE, sendCkbTx, updateCkbTxWithRealBtcTxId } from "@rgbpp-sdk/ckb";
...
const prepareClusterCell = async ({ outIndex, btcTxId, ckbAddress, clusterData, collector, isMainnet, btcTestnetType }: prepareClusterCellParams): Promise<CKBComponents.RawTransactionToSign> => {
...
const createCluster = async ({ ownerRgbppLockArgs, collector, clusterData, isMainnet, btcTestnetType, fromBtcAccount, fromBtcAccountPubkey, btcDataSource, btcService, unisat }: createClusterParams): Promise<TxResult> => {
    ...
    const { txId: btcTxId, rawTxHex: btcTxBytes } = await signAndSendPsbt(psbt, unisat, btcService);
